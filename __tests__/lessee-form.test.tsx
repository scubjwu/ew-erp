...
vi.mock("@/app/partners/lessee/actions", () => ({
  revalidateLesseeViews: vi.fn().mockResolvedValue(undefined),
}));
...
vi.mock("@/lib/lessees/generate-lessee-code", () => ({
  generateLesseeCode: vi.fn(() => "BABCDE"),
}));
...
describe("LesseeForm", () => {
  it("renders create and edit actions and keeps view entrypoint", async () => {
    const { rerender } = render(
      <LesseeForm mode="create" regionOptions={regionOptions} picOptions={picOptions} />
    );
    expect(await screen.findByRole("button", { name: "Create Lessee" })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "Cancel" })).toHaveAttribute("href", "/partners/lessee");

    rerender(
      <LesseeForm
        mode="edit"
        initialLessee={initialLessee}
        regionOptions={regionOptions}
        picOptions={picOptions}
      />
    );
    expect(await screen.findByRole("button", { name: "Save Changes" })).toBeInTheDocument();
    expect(screen.getByRole("textbox", { name: /legal company name/i })).toHaveValue("Lessee One");
    ...
    expect(screen.getByRole("link", { name: /Edit Lessee/i })).toBeInTheDocument();
  });
});
