  ...
  const updates = Array.from(grouped.entries()).map(async ([rowId, patch]) => {
    try {
      ...
      const { data: currentRow, error: fetchError } = await supabase
        .from("inventory")
        .select("sales, customer_id, target_price, status, condition, container_size, container_type, container_specs, logistics_data, financial_data, remarks")
        .eq("id", rowId)
        .single();
...
export async function fetchInventoryData(
  ...
  if (params) {
    ...
    if (tf.color) query = query.ilike("container_specs->>color_code", `%${tf.color.trim()}%`);
    if (tf.engine) query = query.ilike("container_specs->>engine", `%${tf.engine.trim()}%`);
    if (tf.pol) query = query.ilike("logistics_data->>pol", `%${tf.pol.trim()}%`);
    if (tf.pod) query = query.ilike("logistics_data->>pod", `%${tf.pod.trim()}%`);
    ...
    if (df.etaFrom) query = query.gte("logistics_data->>eta", df.etaFrom);
    if (df.etaTo) query = query.lte("logistics_data->>eta", df.etaTo);
  ...
  return (data ?? []).map((row: any) => {
    ...
    return {
      ...
      depotName: row.logistics_data?.planned_depot_Name || "",
      depotAddr: row.logistics_data?.planned_depot_Addr || "",
      depotTel: row.logistics_data?.planned_depot_Tel || "",
      gateInRef: row.logistics_data?.gate_in_ref || "",
      transitCompany: row.logistics_data?.lessee || "",
      pol: row.logistics_data?.pol || "",
