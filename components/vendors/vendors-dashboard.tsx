...
const SORTABLE_COLUMNS: Array<SortableColumnConfig<VendorSortBy>> = [
  {
    key: "vendorCode",
    label: "Vendor Code",
    sortable: true,
  ...
  },
  {
    ...
    sortable: true,
    sortKey: "legalCompanyName",
    widthClass: "min-w-[220px]",
  },
  {
    key: "companyName",
    label: "Company Name (Other Language)",
    sortable: true,
    sortKey: "companyName",
    widthClass: "min-w-[220px]",
  },
  ...
  {
    key: "currentPrepaidBalance",
    label: "Current Prepaid Balance",
    sortable: true,
    sortKey: "currentPrepaidBalance",
    align: "right",
...
function SortButton({
  ...
  label: string;
  sortKey: VendorSortBy;
  activeSortBy: VendorSortBy;
  activeDirection: VendorSortDirection;
  onToggle: (key: VendorSortBy) => void;
}) {
...
function findRegionOptionById(options: VendorAutocompleteOption[], value: string) {
  return options.find((option) => option.value === value) ?? null;
}

export function VendorsDashboard({ initial, pageSize, filterOptions }: Props) {
  ...
  function toggleSort(key: VendorSortBy) {
    const nextSort = {
      sortBy: key,
      sortDirection:
        sort.sortBy === key && sort.sortDirection === "asc" ? "desc" : "asc",
    } as const;
  ...
  function resetFilters() {
    ...
    void refresh(EMPTY_FILTERS, 1, DEFAULT_VENDOR_SORT);
  }

  function updateTextFilter(
    field: "vendorCode" | "legalCompanyName",
    value: string
  ) {
  ...
  function selectRegion(option: VendorAutocompleteOption | null) {
    setAutocompleteInputs((current) => ({
      ...current,
      ...
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-4 px-4 py-6 md:px-6 lg:px-8">
        <div className="rounded-xl border bg-card p-4 shadow-sm">
          <StandardListPageHeader
            title="Vendors"
            description="Maintain vendor master data, bank details, settlement terms, and attachments."
            actions={
              <>
          ...
          >
            <AutocompleteFilterInput
              ...
              onSelect={(option) => updateTextFilter("vendorCode", option?.value ?? "")}
              onClear={() => updateTextFilter("vendorCode", "")}
              emptyMessage="No matching vendor codes."
              disabled={loading}
            />
            ...
            <AutocompleteFilterInput
              ...
              onSelect={(option) => updateTextFilter("legalCompanyName", option?.value ?? "")}
              onClear={() => updateTextFilter("legalCompanyName", "")}
              emptyMessage="No matching company names."
              disabled={loading}
            />
        ...
        <div className="rounded-xl border bg-card shadow-sm">
              ...
              <TableHeader className="sticky top-0 z-10 bg-card">
                <TableRow>
                  {SORTABLE_COLUMNS.map((column) => (
                    <TableHead
                      key={column.key}
                      className={[column.widthClass ?? "", alignClass(column.align)]
                        .filter(Boolean)
                        .join(" ")}
                    >
                ...
                {result.rows.length === 0 ? (
                  ...
                  result.rows.map((vendor) => (
                    <TableRow key={vendor.id}>
                      <TableCell className="min-w-[150px] font-medium">{vendor.vendor_code}</TableCell>
                      <TableCell className="min-w-[220px]">{vendor.legal_company_name}</TableCell>
                      <TableCell className="min-w-[220px]">{vendor.company_name ?? "-"}</TableCell>
                      <TableCell className="min-w-[180px]">
                        {vendor.primary_contact_person ?? "-"}
                      </TableCell>
                      <TableCell className="min-w-[220px]">{vendor.contact_email ?? "-"}</TableCell>
                      <TableCell className="min-w-[140px]">{vendor.contact_tel ?? "-"}</TableCell>
                      <TableCell className="min-w-[140px]">
                        {vendor.region?.region_code ?? "-"}
                      </TableCell>
                      ...
                      <TableCell className="min-w-[170px] text-right">
                        {vendor.settlement_current_prepaid_balance == null
                          ? "-"
                          : vendor.settlement_current_prepaid_balance.toFixed(2)}
                      </TableCell>
          ...
          <StandardTablePagination
            summary={`Showing ${start}-${end} of ${result.totalCount} vendors`}
            page={page}
