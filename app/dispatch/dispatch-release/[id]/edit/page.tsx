...
export default async function EditDispatchReleasePage({
  ...
  const editData: DispatchReleaseEditInitialData = {
    ...
    selectedContainers: activeItems.map((item) => ({
      ...
      gateInRef: item.gateInRef,
      returnDepotName: item.returnDepotName,
      returnDepotAddress: item.returnDepotAddress,
      returnDepotTel: item.returnDepotTel,
      arrangeDate: item.arrangeDate ? new Date(item.arrangeDate) : null,
      customerOrderNum: item.customerOrderNum,
